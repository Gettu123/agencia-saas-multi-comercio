(function () {
  ...
  function applyCopy() {
    els.i18n.forEach(function (node) { node.textContent = t(node.dataset.i18n); });
  }
  async function boot() {
    const validation = await AuthEngine.validateTenant(tenantId);
    if (!validation.active) {
      location.replace("./login.html?tenant=" + encodeURIComponent(tenantId) + "&blocked=1");
      return;
    }
  ...
  els.form.addEventListener("submit", function (ev) {
    ...
    saveBookings(items);
    renderBookings();
    const gateway = PaymentGateways[config.gateway];
    const payload = gateway.generatePayload({
      amount: service.guarantee,
      reference: booking.service + " \u00b7 " + booking.client,
      merchant: config.brand.name
    });
    gateway.renderPanel(els.pay, payload);
    els.client.value = "";
  });
  ...
  boot().catch(function (err) {
    document.body.innerHTML = '<main class="shell"><p>Erro ao iniciar o app: ' + err.message + '</p></main>';
  });
})();
